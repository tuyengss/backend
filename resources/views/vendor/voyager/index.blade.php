...
@section('javascript')
    ...
    @if($role_id != 5 && $role_id != 6)
        ...
        var data = google.visualization.arrayToDataTable([
          ['', 'LEAD','Account', 'HS vay', 'HS Success'],
          <?php foreach ($history_col_chart as $key => $value) :?>
          ['{{'Tháng '.$value->Thang}}', {{$value->lead}}, {{$value->account}} ,{{$value->ho_so_vay}}, {{$value->duocduyet}}],
          <?php endforeach ?>
        ]);
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['', 'LEAD','Account', 'HSV', 'HS Success'],
         <?php foreach ($history_line_chart as $key => $val):?>
          ['{{$val->Ngay.'/'.$val->Thang.'/2018'}}',{{ $val->lead }} , {{ $val->account }} , {{ $val->ho_so_vay }} ,{{$val->duocduyet}}],
          <?php endforeach ?>
        ]);
