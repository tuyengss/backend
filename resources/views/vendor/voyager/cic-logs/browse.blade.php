                        ...
                        <div class="table-responsive">
                            <table id="dataTable" class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="no-sort no-click" id="bread-actions">
                                            @foreach(Voyager::actions() as $action)
                                                ...
                                                @else
                                                    <a class="btn btn-sm btn-warning pull-right view" href="{{url('admin/log-cic',$data->id)}}"><i class="voyager-eye"></i> view</a>
                                                @endif
                                               
                                            @endforeach
                                             @if($data->check_admin === 1)
                                                    <a type="button" class="btn btn-success btn-add-new" disabled><i class="voyager-check"></i> Checked</a>
                                                @endif
                                        </td>
                                    </tr>
                                    @endif {{--end test if admin || admin  Administrator --}}
                                    
                                    @if(Auth::user()->role_id != 1 && Auth::user()->role_id != 4) {{-- test if admin || admin  Administrator --}}
                                    @if(Auth::user()->id === $data->user_id) {{-- set user id login == user_id in data --}}
                                    <tr>
                                        @can('delete',app($dataType->model_name))
                                            <td>
                                                <input type="checkbox" name="row_id" id="checkbox_{{ $data->getKey() }}" value="{{ $data->getKey() }}">
                                    ...
                                    @endif  {{-- end set user id login == user_id in data --}}
                                    @endif {{--end test if admin || admin  Administrator --}}
                                    @endforeach
                                </tbody>
                            </table>
